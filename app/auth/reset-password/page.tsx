...
export default function ResetPasswordPage() {
  ...
  // Firebase передаёт код в query: ?mode=resetPassword&oobCode=XXXX
  ...
  // Проверяем валидность кода при загрузке страницы
  useEffect(() => {
    if (!oobCode) {
    ...
    verifyPasswordResetCode(auth, oobCode)
      ...
      .catch(() => {
        setError('Ссылка истекла или уже использована. Запросите новую.')
        setVerifying(false)
      })
  }, [oobCode])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Аналог: supabase.auth.updateUser({ password })
