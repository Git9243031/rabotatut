...
export default function EditJobPage() {
  ...
  useEffect(() => {
    if (!id) return
    jobsService.getJob(id).then(job => {
      if (!job) { router.push('/dashboard/hr'); return }
      reset({
        title:            job.title,
        company:          job.company,
        ...
        skills:           job.skills            ?? [],
        contact_tg:       job.contact_tg        ?? '',
      })
      setSphere(job.sphere       ?? '')
      setSubSphere(job.sub_sphere ?? '')
      setCity(job.location       ?? 'Удалённо')
      setLoading(false)
    })
  }, [id, reset, router])
    ...
    <div className="min-h-screen bg-[#F8FAFC] py-8">
      <div className="max-w-2xl mx-auto px-4 sm:px-6">
        ...
        <div className="bg-white rounded-[20px] border border-[#E5E7EB] p-6 sm:p-8">
          <h1 className="text-xl font-bold text-[#0F172A] mb-6">Редактировать вакансию</h1>
